"""
This field mostly base on https://github.com/Gorash/odoo-view-pocalypse-convert/ of odoo
========================================================================INSTRUCTION========================================================================
1. First you need to add this into odoo-config: --upgrade-path=~/git/upgrade-util/src
2. Then run odoo 16, install all needed module to migrate view
3. Then create a migration for any module that you have installed, but that one need to be end-migration because we need self.env[model_name]
4. Copy this code into you end-migration script (don't forget to increase manifest version also)
...
script = util.import_script("base/17.0.1.3/duong_view_appocalypse.py")
root_directory = '/home/duongnguyen/git/odoo-tvtma/odoo-tvtma16' # Name your directory contain odoo code here, you can switch to list to support multi and don't forget to modified the code below


def migrate_view_appocalypse(cr):
    # Function to process XML files and extract record id
    def view_appocalypse(folder_name, xml_path):
        try:
            tree = etree.parse(xml_path)
            root = tree.getroot()

            # Find the record element with the specified attributes
            record_elems = root.findall(".//record[@model='ir.ui.view']")

            if record_elems is not None:
                for record_elem in record_elems:
                    record_id = record_elem.get('id')
                    if not record_id:
                        continue
                    xml_id = f"{folder_name}.{record_id}"
                    vid = util.ref(cr, xml_id)
                    IrUiView = util.env(cr)["ir.ui.view"]
                    view = IrUiView.browse(vid)
                    if view.exists():
                        xml_filename = xml_path.replace(root_directory+"/", "")
                        script.convert_ir_ui_view_modifiers(view, record_elem, xml_filename)

        except ET.ParseError as e:
            print(f"Error parsing XML file {xml_file}: {e}")

    # Traverse through each subfolder containing 'views'
    result = {}
    for subdir, dirs, files in os.walk(root_directory):
        if 'views' in dirs:
            views_folder = os.path.join(subdir, 'views')
            xml_files = [f for f in os.listdir(views_folder) if f.endswith('.xml')]
            for xml_file in xml_files:
                xml_path = os.path.join(views_folder, xml_file)
                folder_name = os.path.basename(subdir)
                view_appocalypse(folder_name, xml_path)
        if 'wizard' in dirs:
            wizard_folder = os.path.join(subdir, 'wizard')
            xml_files = [f for f in os.listdir(wizard_folder) if f.endswith('.xml')]
            for xml_file in xml_files:
                xml_path = os.path.join(wizard_folder, xml_file)
                folder_name = os.path.basename(subdir)
                view_appocalypse(folder_name, xml_path)


def migrate(cr, version):
    migrate_view_appocalypse(cr)
# CODE: END

5. After complete end-migration, you will see from the log that some view might have incomplete from view migration, you need to manual do that.
6. After manual fix and check, you can commit the result (if you using github) then copy commit id, from there cherry-pick it into odoo 17.0
"""
...
from odoo.tools import mute_logger, locate_node, apply_inheritance_specs
from odoo.exceptions import ValidationError
from odoo.tools.misc import unique, file_open
...
from odoo.tools.safe_eval import assert_valid_codeobj, _BUILTINS
...
def convert_ir_ui_view_modifiers(self, record_node, xml_path, extra_vals=None):
    ...
    try:
        ...
        if data_id:
            if '.' not in data_id:
                data_id = f'{self._module}.{data_id}'
        ...
        # load previous arch
        arch = None
        previous_xml = file_open(xml_path, 'r').read()
        match = re.search(rf'''(<record [^>]*id=['"]{rec_id}['"][^>]*>(?:[^<]|<(?!/record>))+</record>)''', previous_xml)
        if not match:
            _logger.error(f"Can not found {rec_id!r} from {xml_path}")
            return
        record_xml = match.group(1)

        match = re.search(rf'''(<field [^>]*name=["']arch["'][^>]*>((.|\n)+)</field>)''', record_xml)
        if not match:
            _logger.error(f"Can not found arch of {rec_id!r} from {xml_path}")
            return
        ...
        try:
            ...
            _logger.error(f"Can not convert: {rec_id!r} from {xml_path}\n{e}")
            return

        if re.sub(rf'(\n| )*{reg_comment}(\n| )*', '', arch_result) == '<data></data>':
            _logger.error(f'No uncommented element found: {rec_id!r} from {xml_path}')
        ...
        if arch_result != arch:
            ...
            with file_open(xml_path, 'w') as file:
                file.write(new_xml)
            try:
                # test file before save
                etree.fromstring(new_xml.encode())
            except Exception as e:
                _logger.error(f'Wrong view conversion in {rec_id!r} from {xml_path}\n\n{arch}\n\n{e}')
                return
...
def _modifier_to_domain_ast_leaf(item_ast, should_contain_domain=False, need_parenthesis=False):
    ...
    if isinstance(item_ast, ast.Compare):
        ...
        if len(item_ast.ops) > 1:
            raise ValueError(f"Should not more than one comparaison: {item_ast.ops}")
